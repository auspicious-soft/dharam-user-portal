...
interface DropZone {
  ...
  displayText: string; // The text shown on the right
}
     ...
     <div className="grid grid-cols-[1fr_1.4fr] lg:grid-cols-2 gap-4">
              ...
              <div className="flex flex-wrap flex-col gap-4">
                {availableItems.map((item) => (
                  <div
                    ...
                    className="max-w-52 w-full px-1 md:px-5 py-2.5 bg-light-blue rounded-lg  border-[1px] border-dashed border-paragraph text-paragraph text-xs md:text-sm  font-medium cursor-move transition-all touch-none"
                  >
                    <div>{item.text}</div>
                  </div>
                ))}
        ...
        <div className="space-y-3">
          {question.dropZones.map((zone) => {
            ...
            const isCorrect =
              showResult && droppedItemId === zone.correctItemId;
            const isWrong =
              showResult &&
              droppedItemId &&
              droppedItemId !== zone.correctItemId;
              ...
              <div key={zone.id} className="flex items-center gap-1 md:gap-4">
                ...
                <div className="max-w-52 w-full flex-shrink-1">
                  <div
                    ...
                    ${droppedItem && !showResult ? "!border-primary_blue bg-[#F0F8FF] text-primary_blue" : ""}
                    ${isCorrect ? "border-solid border-[#D2FFC9] bg-[#D2FFC9]" : "text-paragraph"}
                    ${isWrong ? "border-solid border-[#FFE6E6] bg-[#FFE6E6]" : "text-Black_light"}
                  `}
                  >
                    {droppedItem ? (
                      <>
