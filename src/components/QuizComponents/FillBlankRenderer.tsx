      ...
      <div className="justify-start text-paragraph text-sm md:text-base leading-6">
        {parts.map((part, idx) => {
          ...
          if (match) {
            ...
            const isWrongAnswer =
              showResult && selectedText && selectedText !== correctAnswer;
              ...
              <span key={idx} className="inline-flex items-baseline mx-1 ">
                <span
                  className={`inline-block min-w-[120px] text-center px-1 py-1 mb-3 border-b-1 transition-all text-paragraph text-[15px] leading-6 min-h-8 
                    ...
                    ${isCorrectAnswer ? " bg-[#D2FFC9] !text-Black_light border-b-[1px] border-dashed border-[#D2FFC9]" : ""} 
                    ${isWrongAnswer ? "!bg-[#FFE6E6] !border-[#FFE6E6] border-b-[1px] border-dashed" : ""}
                  `}
                >
                  {selectedText || ""}
                </span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h4 className="text-paragraph text-sm md:text-base font-semibold">Options:</h4>
        <div className="space-y-3">
          {question.options.map((option, optionIndex) => {
            const selectedBlank = currentAnswers[optionIndex] || "";
            const correctStatus = showResult ? isCorrect(optionIndex) : null;

            return (
              <div key={optionIndex} className="flex items-center gap-4">
                {/* Option Text (Left Side) */}
                <div
                  className={`flex-1 px-4 py-3 rounded-lg border-[1px] text-paragraph text-xs md:text-sm font-medium transition-all
                    ${!showResult && !selectedBlank ? "border-[#dce5ed] bg-white text-paragraph" : ""}
                    ${!showResult && selectedBlank ? "!border-primary_blue bg-[#F0F8FF] text-primary_blue" : ""}
                    ${correctStatus === true ? "border-[#D2FFC9] bg-[#D2FFC9] !text-Black_light" : ""}
                    ${correctStatus === false ? "border-[#FFE6E6] bg-[#FFE6E6] text-paragraph" : ""}
                  `}
                >
                  {option}
                </div>
                ...
                <div className="flex items-center gap-3 min-w-[140px]">
                  ...
                  <div className="relative flex-1">
                    <select
                      ...
                      disabled={showResult}
                      className={`w-full px-4 py-3 pr-10 rounded-lg border-[1px] outline-none transition-all text-paragraph text-xs md:text-sm font-medium appearance-none cursor-pointer
                        ${!showResult && !selectedBlank ? "border-[#E0E0E0] text-gray-400" : ""}
                        ${!showResult && selectedBlank ? "!border-primary_blue bg-[#F0F8FF] text-primary_blue" : ""}
                        ${correctStatus === true ? "border-[#D2FFC9] bg-[#D2FFC9] !text-Black_light" : "text-paragraph"}
                        ${correctStatus === false ? "border-[#FFE6E6] bg-[#FFE6E6] text-paragraph" : ""}
